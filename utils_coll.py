...
def getColl(words):
    ...
    for key, value in words.items():
        ...
        if parent_id == -1:
            continue
        parent_cont = words[parent_id]['token']
        ...
        if value['relate'] in ['VOB', 'FOB', 'IOB'] and value['pos'] in ['n', 'ni', 'ns', 'nt', 'nz']:
            if words.__contains__(parent_id + 1) and words[parent_id + 1]['relate'] in ['RAD', 'CMP'] and \
                ...
                if words.__contains__(parent_id + 2) and words[parent_id + 2]['relate'] in ['RAD', 'CMP'] and \
                    ...
                    coll_v_hv_o = parent_cont + '\t' + words[parent_id + 1]['token'] + '\t' + value[
                        'token'] + '\t' + 'V_HV_O'
        ...
        if value['relate'] == 'SBV' and value['pos'] not in ['r', 'nh', 'nl']:
            if words.__contains__(parent_id + 1) and words[parent_id + 1]['relate'] in ['RAD', 'CMP'] and \
                ...
                if words.__contains__(parent_id + 2) and words[parent_id + 2]['relate'] in ['RAD', 'CMP'] and \
                    ...
                    coll_s_v_hv = value['token'] + '\t' + parent_cont + '\t' + words[parent_id + 1][
                        'token'] + '\t' + 'S_V_HV'
                    collocation.append(coll_s_v_hv)
            elif words[parent_id - 1]['token'] not in [':', '：']:
        ...
        if value['relate'] == 'ADV' and value['pos'] in ['a', 'd', 'v']:
            ...
            elif key < parent_id and words[key + 1]['token'] == '地':
            ...
            if key == parent_id + 1:
                if words.__contains__(key + 1):
                    if words[key + 1]['token'] in ['了', '得', '过']:
                        coll_v_c_u = parent_cont + '\t' + value['token'] + '\t' + words[key + 1]['token'] + '\t' + 'V_C_U'
                ...
                if words[parent_id + 1]['token'] in ['了', '得', '过']:
                    if key == parent_id + 3:
                        if words[key - 1]['relate'] == 'ATT':
                            ...
                            coll_v_u_x_c = parent_cont + '\t' + words[key - 2]['token'] + '\t' + 'X' + '\t' + value[
                                'token'] + '\t' + 'V_U_X_C'
                            collocation.append(coll_v_u_x_c)
                    else:
                        coll_v_u_x_c = parent_cont + '\t' + words[parent_id + 1]['token'] + '\t' + 'X' + '\t' + value[
                            'token'] + '\t' + 'V_U_X_C'
                        collocation.append(coll_v_u_x_c)
                else:
