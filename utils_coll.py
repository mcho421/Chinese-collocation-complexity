...
def getColl(dependency_tree):
    ...
    for token_idx, value in dependency_tree.items():
        parent_idx = int(value['parent'])
        if parent_idx == -1:
            continue
        parent_cont = dependency_tree[parent_idx]['token']

        if value['pos'] in ['wp', 'ws'] or dependency_tree[parent_idx]['pos'] in ['wp', 'ws']:
            continue
        ...
        if value['relation'] in ['VOB', 'FOB', 'IOB'] and value['pos'] in ['n', 'ni', 'ns', 'nt', 'nz']:
            if dependency_tree.__contains__(parent_idx + 1) and dependency_tree[parent_idx + 1]['relation'] in ['RAD', 'CMP'] and \
                ...
                if dependency_tree.__contains__(parent_idx + 2) and dependency_tree[parent_idx + 2]['relation'] in ['RAD', 'CMP'] and \
                    ...
                    coll_v_hv_o = parent_cont + '\t' + dependency_tree[parent_idx + 1]['token'] + '\t' + value[
                        'token'] + '\t' + 'V_HV_O'
        ...
        if value['relation'] == 'SBV' and value['pos'] not in ['r', 'nh', 'nl']:
            if dependency_tree.__contains__(parent_idx + 1) and dependency_tree[parent_idx + 1]['relation'] in ['RAD', 'CMP'] and \
                ...
                if dependency_tree.__contains__(parent_idx + 2) and dependency_tree[parent_idx + 2]['relation'] in ['RAD', 'CMP'] and \
                        dependency_tree[parent_idx + 2]['token'] not in notHV:
                    coll_s_v_2hv = value['token'] + '\t' + parent_cont + '\t' + dependency_tree[parent_idx + 1]['token'] + '\t' + \
                                   dependency_tree[parent_idx + 2]['token'] + '\t' + 'S_V_2HV'
                    collocation.append(coll_s_v_2hv)
                else:
                    coll_s_v_hv = value['token'] + '\t' + parent_cont + '\t' + dependency_tree[parent_idx + 1][
                        'token'] + '\t' + 'S_V_HV'
        ...
        if value['pos'] == 'p' and value['token'] in prepositions:
            if value['relation'] == 'ADV' and dependency_tree[parent_idx]['pos'] == 'v' and token_idx < parent_idx:
                if dependency_tree.__contains__(parent_idx + 1) and dependency_tree.__contains__(parent_idx + 2) and dependency_tree[parent_idx + 1][
                    ...
                    coll_p_v_2hv = value['token'] + '\t' + 'X' + '\t' + parent_cont + '\t' + dependency_tree[parent_idx + 1][
                    ...
                    collocation.append(coll_p_v_2hv)
                        ...
                        dependency_tree[parent_idx + 1]['token'] not in notHV:
                    coll_p_v_hv = value['token'] + '\t' + 'X' + '\t' + parent_cont + '\t' + dependency_tree[parent_idx + 1][
                        'token'] + '\t' + 'P_X_V_HV'
            ...
            if token_idx == parent_idx + 1:
                ...
                if dependency_tree[parent_idx + 1]['token'] in ['了', '得', '过']:
                    if token_idx == parent_idx + 3:
                        ...
                        coll_v_u_x_c = parent_cont + '\t' + dependency_tree[parent_idx + 1]['token'] + '\t' + 'X' + '\t' + value[
                            'token'] + '\t' + 'V_U_X_C'
                        collocation.append(coll_v_u_x_c)
                else:
