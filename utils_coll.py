...
def getColl(dependency_tree):
    ...
    for key, value in dependency_tree.items():
        ...
        if value['relation'] == 'POB' and value['pos'] == 'nd':
            if key > parent_id + 1 and dependency_tree[parent_id]['pos'] == 'p' and parent_cont in ['在', '到', '从', '自', '自从', '向',
                                                                                          '往', '除了', '于', '沿着', '至',
                                                                                          '由', '顺着', '朝', '朝着', '沿',
                                                                                          '向着']:
        ...
        if value['relation'] == 'POB' and value['token'] in ['时', '时候']:
            if key > parent_id + 1 and dependency_tree[parent_id]['pos'] == 'p' and parent_cont in ['在', '当', '每当', '从', '自从',
                                                                                          '自', '到']:
                coll_p_dn = parent_cont + '\t' + 'X' + '\t' + value['token'] + '\t' + 'P_X_DN'
                collocation.append(coll_p_dn)
        ...
        if value['relation'] == 'RAD' and value['pos'] == 'u' and value['token'] not in notHV:
            if key > parent_id + 1 and dependency_tree[parent_id]['pos'] == 'p':
        ...
        if value['relation'] == 'ATT' and value['pos'] == 'a':
            if dependency_tree[parent_id]['pos'] in ['n', 'ni', 'ns', 'nt', 'nz']:
                ...
                elif key == parent_id - 2 and dependency_tree[parent_id - 1]['token'] == '的':
                    coll_a_de_n = value['token'] + '\t' + '的' + '\t' + parent_cont + '\t' + 'A_DE_N'
                    collocation.append(coll_a_de_n)
                elif dependency_tree[key + 1]['token'] == '的':
        ...
        if value['relation'] == 'ADV' and value['pos'] in ['a', 'd', 'v']:
            ...
            elif key < parent_id and dependency_tree[key + 1]['token'] == '地':
        ...
        if value['pos'] == 'p' and value['token'] in prepositions:
            if value['relation'] == 'ADV' and dependency_tree[parent_id]['pos'] == 'v' and key < parent_id:
        ...
        if value['relation'] == 'CMP':
            if key == parent_id + 1:
                if dependency_tree.__contains__(key + 1):
                    if dependency_tree[key + 1]['token'] in ['了', '得', '过']:
                        coll_v_c_u = parent_cont + '\t' + value['token'] + '\t' + dependency_tree[key + 1]['token'] + '\t' + 'V_C_U'
                ...
                if dependency_tree[key - 1]['token'] in ['了', '得', '过']:
                    coll_v_u_c = parent_cont + '\t' + dependency_tree[key - 1]['token'] + '\t' + value['token'] + '\t' + 'V_U_C'
                    collocation.append(coll_v_u_c)
                elif dependency_tree[key - 1]['relation'] == 'ADV':
                    coll_v_d_c = parent_cont + '\t' + dependency_tree[key - 1]['token'] + '\t' + value['token'] + '\t' + 'V_D_C'
                    collocation.append(coll_v_d_c)
                elif dependency_tree[key - 1]['relation'] == 'ATT':
                    if dependency_tree[key - 1]['pos'] == 'm':
                ...
                if dependency_tree[parent_id + 1]['token'] in ['了', '得', '过']:
                    if key == parent_id + 3:
                        if dependency_tree[key - 1]['relation'] == 'ATT':
                            if dependency_tree[key - 1]['pos'] == 'm':
                                coll_v_u_m_c = parent_cont + '\t' + dependency_tree[key - 2]['token'] + '\t' + 'm' + '\t' + value[
                                    'token'] + '\t' + 'V_U_M_C'
                                collocation.append(coll_v_u_m_c)
                            else:
                                coll_v_u_a_c = parent_cont + '\t' + dependency_tree[key - 2]['token'] + '\t' + dependency_tree[key - 1][
                                    'token'] + '\t' + value['token'] + '\t' + 'V_U_A_C'
                                collocation.append(coll_v_u_a_c)
                        elif dependency_tree[key - 1]['relation'] == 'ADV':
                            if value['pos'] != 'v':
                                coll_v_u_d_c = parent_cont + '\t' + dependency_tree[key - 2]['token'] + '\t' + dependency_tree[key - 1][
                                    'token'] + '\t' + value['token'] + '\t' + 'V_U_D_C'
                                collocation.append(coll_v_u_d_c)
                            elif dependency_tree[key + 1]['pos'] == 'wp':
                                coll_v_u_d_c = parent_cont + '\t' + dependency_tree[key - 2]['token'] + '\t' + dependency_tree[key - 1][
                                    'token'] + '\t' + value['token'] + '\t' + 'V_U_D_C'
                                collocation.append(coll_v_u_d_c)
                        else:
                            coll_v_u_x_c = parent_cont + '\t' + dependency_tree[key - 2]['token'] + '\t' + 'X' + '\t' + value[
                                'token'] + '\t' + 'V_U_X_C'
